...
for ((i=0;i<${#R1[@]};i++)); do
        sample=${R1[i]%$suffix}
#       (bowtie2 -N 1 -p $threads -x $hg38 -1 ${R1[i]} -2 ${R2[i]} -S $sample.hg38.sam)2>$sample.hg38.log
#	(bwa mem -t $threads $hg38 ${R1[i]} ${R2[i]} > $sample.hg38.sam)2>$sample.hg38.log
...
R1=(*.hg38.sam.bam.sorted.bam.unmapped.bam_1.fq)
R2=(*.hg38.sam.bam.sorted.bam.unmapped.bam_2.fq)

suffix=".hg38.sam.bam.sorted.bam.unmapped.bam_1.fq"

for ((i=0;i<${#R1[@]};i++)); do
	sample=${R1[i]%$suffix}
        (STAR --genomeDir $X --readFilesIn ${R1[i]} ${R2[i]} --outFilterMismatchNmax 2 --outFilterScoreMinOverLread 0 --outFilterMatchNminOverLread 0 --seedSearchStartLmax 20 --outSAMunmapped Within --outFileNamePrefix $sample.chimeras. --runThreadN $threads)2>$sample.chimera.log
done
...
rm -rf *.hg38.log *.hg38.sam *.hg38.sam.bam *.hg38.sam.bam.sorted.bam.B.bam *.hg38.sam.bam.sorted.bam.L.bam *.hg38.sam.bam.sorted.bam.R.bam *.chimeras.Aligned.out.sam *.chimeras.Aligned.out.sam.bam *.chimeras.Aligned.out.sam.bam.sorted.bam.coverage *.chimeras.Aligned.out.sam.bam.sorted.bam.junction.coverage *.chimera.log *.Log.* *.out.tab chimeras.tsv exon_exon_chimeras_sequence.fnn
